...
def run(total_epoch, best_loss, data_loader): #, tensorboard_writer):
    ...
    for step in range(nepoch + 1, 121):
        ...
        if total_loss < prev_loss:
            ...
            best_model=moddir+'mod-1-{:03d}-transformer'.format(step)

            print("saving:",best_model)
            torch.save(model.state_dict(), best_model)
            lrate=moddir+'lr-1-{:03d}-transformer'.format(step)
            print("saving:",lrate)
            torch.save(optimizer.state_dict(), lrate)
        else:
            worst_model=moddir+'mod-1-{:03d}-transformer'.format(step)
