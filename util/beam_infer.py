...
beam_results, beam_scores, timesteps, out_lens = decoder_tr.decode(emission,num_processes=1)
'''
...
if bpe_flag== True:
    ...
    '''
    decoder=[]
    for w_ins in [0,-1,-1,-1,-2,-2]: #valori positivi aumentano le inserzioni
...
def ctc_cuda_predict(emission, index=5, tokens=tokens):
    ...
    enc_len=torch.full(size=(emission.size(0),), fill_value = emission.size(1), dtype=torch.int32).to(device)    
    cuda_decoder = cuda_ctc_decoder(tokens, nbest=1, beam_size=10, blank_skip_threshold=0.95)
...
def avg_models(model, path, init, end):
    ...
    best_model=path+'mod-w1-{:03d}-transformer'.format(nepoch)
