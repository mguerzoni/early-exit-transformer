...
def evaluate(model):
    ...
    path = os.getcwd()+'/trained_model/ctc1000/'

    model = avg_models(model, path, 101, 119)
    model.eval()
    ...
    for set_ in ["test-clean"]:
        ...
        test_dataset = torchaudio.datasets.LIBRISPEECH("/stek/data/corpora", url=set_, download=False)
