    ...
    for i,c_batch in enumerate(iterator):
        ...
        for batch_0,batch_1,batch_2,batch_3 in c_batch:
            ...
            for j, enc in enumerate(encoder):
                loss_ctc += (6-j) * ctc_loss(enc.permute(1,0,2),batch_1,ctc_input_len,ctc_target_len).to(device)
            del encoder
...
def run(total_epoch, best_loss, data_loader): #, tensorboard_writer):
    ...
    for step in range(nepoch + 1, 120):
        ...
        #tensorboard_writer.add_scalar('Training loss ctc', total_loss, global_step=step)

        if total_loss < prev_loss:
            prev_loss = total_loss
            best_model=moddir+'mod-w1-{:03d}-transformer'.format(step)

            print("saving:",best_model)
            torch.save(model.state_dict(), best_model)
            lrate=moddir+'lr-w1-{:03d}-transformer'.format(step)
            print("saving:",lrate)
            torch.save(optimizer.state_dict(), lrate)
        else:
            worst_model=moddir+'mod-w1-{:03d}-transformer'.format(step)
